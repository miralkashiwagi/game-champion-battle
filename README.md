...
現在の正式VRMA素材は、リポジトリに存在する次の19本です。新しいファイルを前提にせず、足りない専用モーションは `src/client/vrma-motion-registry.js` で既存clipへの明示fallbackとして登録します。
...
VRMロード後の Silver Knight と Syal は `VrmaCharacterMotionPlayer` を使います。Saladin は現時点では script renderer のままですが、Saladin の全 `motionId` も registry に登録済みです。将来 Saladin 用VRMを追加した場合、同じ対応表を使ってVRMA再生へ切り替えられます。
...
VRMAを差し替える場合は、同じパスのファイルを置き換えるか、`vrma-motion-registry.js` のURLと `fallbackReason` を更新してください。`walk-backward`、`strafe-*`、専用 `thrust`、専用 `death` は現状未配置のため、`scripts/validate-vrma.mjs` では警告として扱います。
