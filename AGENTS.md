...
- モーション作成・調整ではスクリプトモデルの見た目よりVRM Humanoidで自然かつ魅力的に見えることを優先する。スクリプトモデル側の腕や肩が多少不自然になっても、VRMのnormalized Humanoid Boneで破綻しない姿勢を採用する。
- 腕を使うモーションでは`UpperArm`だけを大きく回さず、`Shoulder`、`UpperArm`、`LowerArm`、`Hand`、`chest`、`spine`を連動させる。パンチ、斬撃、ガード、被弾、ダウンでは肩から動き始め、胸と骨盤のひねりで腕の軌道を支える。
- 攻撃モーションでは予備動作、インパクト、フォロースルー、復帰を分け、肩・胸・肘・膝・足首のいずれかが段階ごとに変化するようにする。腕だけが瞬間的に前へ伸びるポーズを避ける。
- 装備を失った後の素手攻撃もVRM Humanoid向けに調整する。素手攻撃は肩、胸、前腕、反対腕、脚を含む全身モーションとして扱い、単一腕ボーンだけのパンチにしない。
...
- VRM Humanoid向けの肩・腕調整では、鎖骨相当の動きとして左右`Shoulder`を積極的に使う。`UpperArm.rotation`の極端な値を避け、必要に応じて`Shoulder`へ回転を分配する。
- 腕を前に出す、振る、守る、のけぞるモーションでは、`chest`または`spine`のひねりを併用する。左右の腕だけを対称に広げた静的なポーズにしない。
- VRMとスクリプトモデルで見え方が衝突する場合は、ゲームロジックとRig/Socket契約を守ったうえでVRMの見た目を優先する。
...
11. 腕を使う攻撃、ガード、素手攻撃がVRM Humanoidで肩、胸、前腕を含む全身モーションになっている。
12. 将来分離予定のキャラクターが別キャラクターの設定ファイルや共有ファクトリーを継承していない。
...
- 装備攻撃と素手攻撃がVRM Humanoid向けに肩、胸、肘、膝、足首を含む全身モーションとして再生される
