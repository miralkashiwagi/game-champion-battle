...
test("スタンVRMAは星エフェクト付きのdizzyを再生する", () => {
  const clips = getVrmaMotionSet().clips;
  assert.equal(clips.stunned.url, "/assets/motions/reaction/dizzy.vrma");
...
test("最後のスタン中だけ頭上の星エフェクトを表示する", () => {
  ...
  for (const state of ["Hitstun", "KneelDown", "AirDamaged", "Down"]) {
    ...
    assert.equal(view.disabledEffect.visible, false, `${state} should not show disabled effect`);
  }

  view.update({ state: "Stunned", facing: -1, worldY: 0 }, 1 / 60, 1.25);
  assert.equal(view.disabledEffect.visible, true);
  view.dispose();
});
