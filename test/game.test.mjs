...
test("Syalは独立したSyal名の初期装備を生成する", () => {
  const sim = new MatchSimulation();
  sim.addPlayer("p1", "syal-player", 1000, "syal");
  const syal = sim.players.get("p1");
  for (const item of Object.values(syal.equipment)) {
    assert.equal(item.originCharacterId, "syal");
    assert.match(item.id, /_syal_/);
    assert.match(item.skillId, /^syal_/);
  }
});

test("SyalとSaladinは同じ性能を別のmotionIdで共有する", async () => {
  const { CHARACTER_SPECS } = await import("../src/shared/characters.ts");
  const saladin = CHARACTER_SPECS.saladin;
  const syal = CHARACTER_SPECS.syal;
  assert.deepEqual(
    syal.combo.map(({ id, motionId, ...attack }) => attack),
    saladin.combo.map(({ id, motionId, ...attack }) => attack)
  );
  assert.match(syal.skills.weapon.motionId, /^syal_/);
  assert.match(saladin.skills.weapon.motionId, /^saladin_/);
  assert.notEqual(syal, saladin);
  assert.notEqual(syal.skills, saladin.skills);
});
...
test("SyalのWindwallもSaladin系の共通フックを実行する", () => {
  const sim = new MatchSimulation();
  ...
  assert.equal(syal.equipment.head.originCharacterId, "syal");
  assert.equal(silver.state, "Hitstun");
  assert.equal(silver.stateTimer, 6);
});
