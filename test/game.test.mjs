...
import { COMMON_GUARD_COUNTER, COMMON_WAKE_UP_ATTACK } from "../src/characters/common.ts";
...
function setAttack(sim, side, attack) {
  sim.setInput(side, { ...idleInput(sim.frame + 1), attack });
  sim.tick();
}

function tapAttack(sim, side = "p1") {
  setAttack(sim, side, true);
  setAttack(sim, side, false);
}

function waitUntilAttackEnds(sim, player) {
  for (let i = 0; i < 180 && player.activeAttack; i++) {
    sim.setInput(player.side, idleInput(sim.frame + 1));
    sim.tick();
  }
  assert.equal(player.activeAttack, null);
}
...
test("Silver Knightの通常攻撃はJ短押しをつなぐと4段目まで順に進む", () => {
  const sim = setup();
  const [silver] = players(sim);

  for (const expected of ["silver_combo_1", "silver_combo_2", "silver_combo_3", "silver_combo_4"]) {
    tapAttack(sim);
    assert.equal(silver.activeActionId, expected);
    waitUntilAttackEnds(sim, silver);
  }
});

test("Silver Knightの通常攻撃は時間を空けると1段目から始まる", () => {
  const sim = setup();
  const [silver] = players(sim);
...
test("Silver Knightの通常攻撃は攻撃中のJ離しで段数だけ進まない", () => {
  const sim = setup();
  const [silver] = players(sim);

  tapAttack(sim);
  assert.equal(silver.activeActionId, "silver_combo_1");
  assert.equal(silver.comboStep, 1);
  ...
  waitUntilAttackEnds(sim, silver);
  tapAttack(sim);
  assert.equal(silver.activeActionId, "silver_combo_2");
});
...
test("武器を失うと共通素手コンボへ戻り、素手長押しはHOLD攻撃せず離した時に通常攻撃する", () => {
  ...
  sim.tick();
  assert.equal(silver.activeActionId, "barehand_1");
  assert.equal(silver.attackName, "Barehand 1");
});
...
test("膝つき中に通常攻撃を受けると即座にダウンする", () => {
  const sim = setup();
  const [silver, saladin] = players(sim);
  silver.position.x = 500;
  saladin.position.x = 560;
  silver.facing = 1;
  saladin.state = "KneelDown";
  saladin.stateTimer = 30;

  sim.startAttack(silver, { id: "kneel-followup", motionId: "kneel-followup", name: "Kneel Followup", damage: 5, range: 90, startupFrames: 0, activeFrames: 2, recoveryFrames: 1, effect: "hitstun", guardPierce: true });
  sim.tick();

  assert.equal(saladin.state, "Down");
  assert.equal(saladin.stateTimer, TICK_RATE);
});

test("ダウン時間切れではその場起き上がりを開始する", () => {
  const sim = setup();
  ...
  sim.tick();
  assert.equal(saladin.state, "GetUp");

  for (let i = 0; i < TICK_RATE; i++) sim.tick();
  assert.equal(saladin.state, "Idle");
});

test("ダウン中は向き方向入力で前転し逆方向入力で後転する", () => {
  const sim = setup();
  const [, saladin] = players(sim);
  saladin.state = "Down";
  saladin.stateTimer = TICK_RATE;
  saladin.position.x = 600;
  saladin.facing = 1;

  sim.setInput("p2", { ...idleInput(1), right: true });
  sim.tick();
  assert.equal(saladin.state, "WakeRoll");
  assert.equal(saladin.attackName, "Forward Roll");
  assert.ok(saladin.velocity.x > 0);
  assert.ok(saladin.position.x > 600);

  saladin.state = "Down";
  saladin.stateTimer = TICK_RATE;
  saladin.position.x = 600;
  saladin.velocity.x = 0;
  saladin.previousInput = idleInput(2);
  sim.setInput("p2", { ...idleInput(3), left: true });
  sim.tick();
  assert.equal(saladin.state, "WakeRoll");
  assert.equal(saladin.attackName, "Back Roll");
  assert.ok(saladin.velocity.x < 0);
  assert.ok(saladin.position.x < 600);
});

test("ダウン中は押下済み方向入力では即ロールしない", () => {
  const sim = setup();
  const [, saladin] = players(sim);
  saladin.state = "Down";
  saladin.stateTimer = TICK_RATE;
  saladin.previousInput = { ...idleInput(1), right: true };
  sim.setInput("p2", { ...idleInput(2), right: true });
  sim.tick();

  assert.equal(saladin.state, "Down");
});

test("ダウン中にJを押すと共通起き上がり攻撃を開始する", () => {
  const sim = setup();
  const [, saladin] = players(sim);
  saladin.state = "Down";
  saladin.stateTimer = TICK_RATE;

  sim.setInput("p2", { ...idleInput(1), attack: true });
  sim.tick();

  assert.equal(saladin.state, "AttackStartup");
  assert.equal(saladin.activeActionId, COMMON_WAKE_UP_ATTACK.motionId);
  assert.equal(saladin.attackName, COMMON_WAKE_UP_ATTACK.name);
  assert.ok(saladin.invulnerableUntilFrame > sim.frame);
});

test("ダウン中にJと左右以外のボタンを押すとその場起き上がりを開始する", () => {
  const sim = setup();
  const [, saladin] = players(sim);
  saladin.state = "Down";
  saladin.stateTimer = TICK_RATE;
  ...
  assert.equal(saladin.state, "GetUp");
  assert.equal(saladin.attackName, "Get Up");
  assert.ok(saladin.invulnerableUntilFrame > sim.frame);
});
