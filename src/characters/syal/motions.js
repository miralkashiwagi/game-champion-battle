export const motionController = {
  stateStyle: {
    moveSpeed: 11, idleBob: .018, idleLean: -.055, idleArmSpread: .16, idleLegSpread: .035,
    moveBob: .045, moveLean: -.12, moveTwist: .08, moveStride: .82, moveArmSwing: .48,
    guardLean: -.14, guardTwist: -.12, guardLeftArmX: -.72, guardLeftArmZ: -.62,
    guardRightArmX: -.58, guardRightArmZ: .42
  },
  applyAttack({ id, phase, bones: b, visualRoot }) {
    ...
    if (id === "syal_spiral_kick") {
      b.hips.rotation.y = -.45 * strike;
      ...
      b.rightFoot.rotation.x = -.32 * strike;
      return;
    }
    if (id === "syal_windwall") {
      ...
      b.leftLowerArm.rotation.z = -.55;
      b.rightLowerArm.rotation.z = .55;
    ...
    const reverse = step % 2 === 0 || id === "syal_guard_counter";
    const crossing = id === "syal_lunar_slash" || id === "syal_forward_cut" || id === "syal_guard_counter";
    ...
    b.leftLowerArm.rotation.z = -.42 - (reverse ? .28 : 0) * strike;
    b.rightLowerArm.rotation.z = .42 + (reverse ? 0 : .28) * strike;
    ...
    if (id === "syal_lunar_slash") visualRoot.rotation.y = strike * .22 * (reverse ? -1 : 1);
  }
};
