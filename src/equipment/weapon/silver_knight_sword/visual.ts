...
const handForwardGripRotation = [-1.5, 0, -.3] as [number, number, number];

export const visual={
  createAttachments({THREE,material,makeBlade}){ const attachments=(()=>{const sword=makeBlade(.98,p.metal,0xc7a553,false); return [{socket:"rightHandGrip",object:sword,position:[.015,.065,.105],rotation:handForwardGripRotation}] satisfies EquipmentAttachment[];})(); for(const entry of attachments) mark(entry.object); return {attachments,motions:{drop:"item_drop",pickup:"item_pickup"}}; },
